...
async function getUniqueSpaceCode(): Promise<string> {
    // Retry a few times to avoid rare collisions
    for (let i = 0; i < 5; i++) {
        const code = generate4DigitCode();
        const existing = await client.space.findFirst({ where: { code } });
        if (!existing) return code;
    }
    // Fallback to a wider range if repeated collisions (extremely unlikely)
    return (Math.floor(1000 + Math.random() * 9000)).toString();
}
...
spaceRouter.post("/", userMiddleware, async (req, res) => {
    ...
    if (!parsedData.data.mapId) {
        const code = await getUniqueSpaceCode();
...
// Resolve a 4-digit code to a space id
spaceRouter.get("/code/:code", async (req, res) => {
    ...
    const space = await client.space.findFirst({ where: { code } });
    if (!space) {
        res.status(404).json({ message: "Room not found" });
        return;
    }
    res.json({ spaceId: space.id, code: space.code, name: space.name });
})